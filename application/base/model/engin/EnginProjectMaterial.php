...
class EnginProjectMaterial extends Base
{

    protected $table = "mk_engin_project_material";
    ...
    public function saveData($data){
        ...
        $obj=$model->where(['project_id'=>$data['pro_id'],'status'=>1])->find();
        $number=Loader::model('base/budget/DefaultProjectCopy')->where(['guid'=>$data['guid']])->value('number');
        ...
        try{
            ...
            foreach ($data['material_id'] as $value){
                ...
                $m_obj['book_number']=$book_number;
                $m_obj['old_id']=$m_obj['guid'];
                $m_obj['project_id']=$data['pro_id'];
                unset($m_obj['guid']);
                unset($m_obj['create_time']);
                unset($m_obj['update_time']);
                unset($m_obj['status']);
                unset($m_obj['id']);
                $m_obj['number']=$number; //默认材料数量为装饰工程数量
                $m_obj['project_data_id']=$data['guid'];
