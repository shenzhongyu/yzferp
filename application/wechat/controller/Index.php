    ...
    public function leaveDetailsAll()
    {
        ...
        if($this->request->isPost()) {
            ...
            if ($assign_data){
                ...
                if(isset($assign_data["department_id"])){
                    $dep=new PersonnelDepartment();
                    $d=$dep->getInfoByGuId($assign_data["department_id"]);
                    if(!empty($d)){
                        $assign_data["department_name"]=$d["department_name"];
                    }
                }
                if(isset($assign_data["jobs_id"])){
                    $dep=new PersonnelJobs();
                    $d=$dep->getInfoByGuId($assign_data["jobs_id"]);
                    if(!empty($d)){
                        $assign_data["jobs_name"]=$d["jobs_name"];
                    }
                }
                return self::showJsonReturnCode(1001,$assign_data->toArray());
            }else{
